...
for i = 3:m-2
for j = 3:n-2
if harris(i,j) > 0.2*Hmax && harris(i,j) > harris(i-1,j-1) && harris(i,j) > harris(i-1,j) && harris(i,j) > harris(i-1,j+1) && harris(i,j) > harris(i,j-1) && harris(i,j) > harris(i,j+1) && harris(i,j) > harris(i+1,j-1) && harris(i,j) > harris(i+1,j) && harris(i,j) > harris(i+1,j+1) && harris(i,j) > harris(i-2,j-2) && harris(i,j) > harris(i-2,j-1) && harris(i,j) > harris(i-2,j) && harris(i,j) > harris(i-2,j+1) && harris(i,j) > harris(i-2,j+2) && harris(i,j) > harris(i+2,j-2) && harris(i,j) > harris(i+2,j-1) && harris(i,j) > harris(i+2,j) && harris(i,j) > harris(i+2,j+1) && harris(i,j) > harris(i+2,j+2) && harris(i,j) > harris(i-1,j-2)  && harris(i,j) > harris(i,j-2) && harris(i,j) > harris(i+1,j-2) && harris(i,j) > harris(i-1,j+2)  && harris(i,j) > harris(i,j+2) && harris(i,j) > harris(i+1,j+2)   
result(i,j) = 1;
...
for i = 3:m-2
for j = 3:n-2
if harrisr(i,j) > 0.2*Hmaxr && harrisr(i,j) > harrisr(i-1,j-1) && harrisr(i,j) > harrisr(i-1,j) && harrisr(i,j) > harrisr(i-1,j+1) && harrisr(i,j) > harrisr(i,j-1) && harrisr(i,j) > harrisr(i,j+1) && harrisr(i,j) > harrisr(i+1,j-1) && harrisr(i,j) > harrisr(i+1,j) && harrisr(i,j) > harrisr(i+1,j+1) && harrisr(i,j) > harrisr(i-2,j-2) && harrisr(i,j) > harrisr(i-2,j-1) && harrisr(i,j) > harrisr(i-2,j) && harrisr(i,j) > harrisr(i-2,j+1) && harrisr(i,j) > harrisr(i-2,j+2) && harrisr(i,j) > harrisr(i+2,j-2) && harrisr(i,j) > harrisr(i+2,j-1) && harrisr(i,j) > harrisr(i+2,j) && harrisr(i,j) > harrisr(i+2,j+1) && harrisr(i,j) > harrisr(i+2,j+2) && harrisr(i,j) > harrisr(i-1,j-2)  && harrisr(i,j) > harrisr(i,j-2) && harrisr(i,j) > harrisr(i+1,j-2) && harrisr(i,j) > harrisr(i-1,j+2)  && harrisr(i,j) > harrisr(i,j+2) && harrisr(i,j) > harrisr(i+1,j+2)   
resultr(i,j) = 1;
...
R_newr=0;
for i=2:size(Rr,2)
    min_distr=100;     %we need to take the max value
    for j=i-1:-1:1
...
for i=1:size(x_cord_newr,2)
    final_mat_rotr(x_cord_newr(i),y_cord_newr(i))=R_newr(i);
end
...
hold on;
plot(x_cord_newr, y_cord_newr, 'rx');
title('Rotated ANMS');
hold off;
